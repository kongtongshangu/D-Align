...
from .vision_transformer_our import VisionTransformer as VisionTransformer_our
...
def _create_vision_transformer_our(variant, pretrained=False, default_cfg=None, **kwargs):
    default_cfg = default_cfg or default_cfgs[variant]
    if kwargs.get('features_only', None):
        raise RuntimeError('features_only not implemented for Vision Transformer models.')
...
def vit_base_patch16_224_our(pretrained=False, **kwargs):
    """ ViT-Base model (ViT-B/16) from original paper (https://arxiv.org/abs/2010.11929).
    ImageNet-1k weights fine-tuned from in21k @ 384x384, source https://github.com/google-research/vision_transformer.
    """
    model_kwargs = dict(patch_size=16, embed_dim=768, depth=12, num_heads=12, **kwargs)
    model = _create_vision_transformer_our('vit_base_patch16_224', pretrained=pretrained, **model_kwargs)
    return model


def create_model_d_align(
        model_name,
        pretrained=False,
        checkpoint_path='',
        scriptable=None,
    ...
    with set_layer_config(scriptable=scriptable, exportable=exportable, no_jit=no_jit):
        model = vit_base_patch16_224_our(pretrained=pretrained, **kwargs)
...
class vit_our(VisionTransformer_our):
    ...
    def forward(self, x, len_keep=None, return_attn=False, return_logits=False):
