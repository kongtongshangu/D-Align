...
class MemoryBank:
    def __init__(self, num_classes, capacity_per_class):
        self.num_classes = num_classes
        self.capacity_per_class = capacity_per_class

        self.features = [None] * num_classes
        self.scores = [None] * num_classes
        self.sizes = torch.zeros(num_classes, dtype=torch.long)
        self.updates_after_full = torch.zeros(num_classes, dtype=torch.long)

    def update(self, batch_features, batch_scores, batch_logits, add_proportion=1):
        y_hat = batch_logits.argmax(dim=1)
        ...
        for c in range(self.num_classes):
            ...
            was_full = (self.sizes[c] == self.capacity_per_class)

            class_features = batch_features[class_mask]
            class_scores = batch_scores[class_mask]
            num_candidates = math.ceil(len(class_features) * add_proportion)
            if num_candidates == 0 and len(class_features) > 0:
                num_candidates = 1

            _, top_indices = torch.topk(class_scores, k=num_candidates, largest=False)
            candidate_features = class_features[top_indices]
            candidate_scores = class_scores[top_indices]
            if self.features[c] is None:
                self.features[c] = torch.zeros(self.capacity_per_class, feature_dim, device='cpu')
                self.scores[c] = torch.full((self.capacity_per_class,), float('inf'), device='cpu')
            for feature, score in zip(candidate_features, candidate_scores):
                if self.sizes[c] < self.capacity_per_class:
                    self.features[c][self.sizes[c]] = feature.detach().cpu()
                    self.scores[c][self.sizes[c]] = score.detach().cpu()
                    self.sizes[c] += 1
                else:
                    max_score_in_class, max_idx = torch.max(self.scores[c], dim=0)
                    if score < max_score_in_class:
                        self.features[c][max_idx] = feature.detach().cpu()
                        self.scores[c][max_idx] = score.detach().cpu()
            if was_full:
                self.updates_after_full[c] += len(candidate_features)

    def is_ready(self):
        num_full_classes = torch.sum(self.sizes == self.capacity_per_class).item()
        return num_full_classes >= (int(self.num_classes * 0.6))
    ...
    def get_contents(self):
        ...
        for c in range(self.num_classes):
            if self.sizes[c] > 0:
                all_features.append(self.features[c][:self.sizes[c]])

        if not all_features:
            return None

        return torch.cat(all_features, dim=0).cuda()

class CorA():
    def __init__(self, model, memory_bank, filter_K=100, W_num_iterations=20, W_lr=0.001, VP_lr=1e-5):
        self.classifier = model.classifier
        self.filter_K = filter_K
    ...
    def _init_params(self, feat_dim, visual_prompt):
        self.W = torch.eye(feat_dim, device='cuda', requires_grad=True)
        self.visual_prompt = visual_prompt
        self.optimizer = torch.optim.Adam([
        {"params": [self.W], "lr": self.W_lr},
        {"params": [self.visual_prompt.left_bar, self.visual_prompt.right_bar], "lr": self.VP_lr}
        ])

    def calculate(self, num_classes, embeddings_arr, logits_arr, score, proportion_vector, visual_prompt):
        if self.W is None:
            self._init_params(embeddings_arr.size(1), visual_prompt)
        all_z = embeddings_arr
        self.num_classes = num_classes
        self.labels = logits_arr
        self.proportion = proportion_vector

        self.memory_bank.update(embeddings_arr, score, logits_arr)
        supports = self.memory_bank.get_contents()
        if not self.memory_bank.is_ready():
            return self.classifier(embeddings_arr)
        self.num_CORAL_C += 1
        z_w = self.linearCORAL(supports, all_z, visual_prompt, num_iterations=self.W_num_iterations, learning_rate=self.W_lr, vp_learning_rate=self.VP_lr)
        chunk_output = self.classifier(z_w)
        return chunk_output

    def coral_loss(self, C_s, C_t):
        d = C_s.size(0)
        return torch.sum((C_s - C_t) ** 2) / (4.0 * d * d)
    ...
    def linearCORAL(self, X_s, X_t, visual_prompt, num_iterations=20, learning_rate=0.001, vp_learning_rate=1e-5):
        ...
        with torch.enable_grad():
            for _ in range(num_iterations):
                ...
                X_t = X_t.detach()
                C_s = C_s.detach()
                C_t_transformed = self.W.T @ C_t @ self.W
                loss = self.coral_loss(C_t_transformed, C_s)
                loss.backward()
                self.optimizer.step()

        X_t_aligned = (X_t - X_t_mean) @ self.W.detach() + X_s_mean

        alpha = 0.7
        return (1 - alpha) * X_t + alpha * X_t_aligned
